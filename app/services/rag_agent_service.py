...
class RagAgentService:
    ...
    def _summarize_exception(error: Exception) -> str:
        """压缩异常信息，避免 ExceptionGroup 日志过长。"""
        exception_group_items = getattr(error, "exceptions", None)
        if exception_group_items and isinstance(exception_group_items, tuple):
            sub_errors = [f"{type(sub).__name__}: {sub}" for sub in exception_group_items[:3]]
            remaining = len(exception_group_items) - len(sub_errors)
            suffix = f"; 其余 {remaining} 个子异常省略" if remaining > 0 else ""
            return f"{error}; 子异常: {' | '.join(sub_errors)}{suffix}"
        return str(error)
    ...
    async def _initialize_agent(self):
        ...
        try:
            ...
            self.mcp_tools = []
            logger.error(
                "加载 MCP 工具失败，降级为基础工具模式: "
                f"{self._summarize_exception(e)}"
            )
        ...
        self.agent = create_agent(
            self.model,#模型
            tools=all_tools,#工具列表
            checkpointer=self.checkpointer,#记忆
        )
    ...
    async def query_stream(
        ...
        try:
            ...
            error_message = self._summarize_exception(e)
            logger.error(f"[会话 {session_id}] RAG Agent 查询失败（流式）: {error_message}")
            yield {
                "type": "error",
                "data": error_message
            }
